/*!
 * Sovereign Flow State Forge v18.14
 *
 * Full production implementation of the Flow State Forge + Dynamic Challenge-Skill Balancer.
...
/// Configuration for the Dynamic Challenge-Skill Balancer.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ChallengeBalancerConfig {
    pub mercy_factor: f32,      // How strongly we lower difficulty when struggling (0.35–0.45)
    pub growth_factor: f32,     // How much we raise when too easy (0.18–0.25)
    pub rhythm_bonus: f32,      // Reward for high rhythm consistency
...
pub fn check_flow_state(metrics: &FlowStateMetrics) -> Option<FlowStateOutcome> {
    ...
    if flow_score > 0.62 {
        ...
        Some(FlowStateOutcome {
            ...
            time_dilation_factor: 0.65 + intensity * 0.7,
            divine_whisper_flavor: "flow_state_revelation".to_string(),
            world_effects,
...
/// Mercy-gated: lowers resistance when struggling (invitation), gently raises when too easy (growth).
