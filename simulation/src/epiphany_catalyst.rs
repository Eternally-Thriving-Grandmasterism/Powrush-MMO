 ...
 *
 * Data-driven epiphany system with registry support for multiple scenarios.
 * Currently supports:
 * - Overflow Lesson (existing live scenario)
 * - Council Harmony (new)
 * - Sustainable Abundance Loop (new)
 *
 * All scenarios are fully TOLC 8 mercy-gated.
 * Designed for easy extension via JSON + registry.
 */
...
/// Registry of available epiphany scenarios (data-driven ready)
pub fn get_available_scenarios() -> Vec<&'static str> {
    vec![
        "overflow_lesson",
        "council_harmony",
        "sustainable_abundance",
    ]
}

/// Existing Overflow Lesson detector (preserved and celebrated)
pub fn check_overflow_lesson(
    depletion: f32,
    sustainable_pacing: bool,
    biome: &str,
) -> Option<EpiphanyOutcome> {
    if !sustainable_pacing || depletion > 0.58 {
        return None;
    }
    ...
    // Add world effects etc.
    Some(outcome)
}

/// New: Council Harmony detector (multiplayer attunement)
pub fn check_council_harmony(
    collective_attunement: f32,
    participant_count: u8,
    duration_ticks: u64,
) -> Option<EpiphanyOutcome> {
    if participant_count < 3 || collective_attunement < 0.55 || duration_ticks < 180 {
        return None;
    }
...
/// New: Sustainable Abundance Loop detector
