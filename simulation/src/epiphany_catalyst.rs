/*!
 * Sovereign Epiphany Catalyst v18.97.1 + Proactive Joy Wiring
 *
 * After strong epiphany outcomes (especially council harmony and high
 * abundance_bloom_multiplier), we now call generate_proactive_joy_redemption_thread()
 * for positive emotional reward loops.
 *
...
use crate::player_legacy_journal::LegacyJournalRegistry; // NEW: proactive joy

// ... (all existing structs preserved)
...
pub fn evaluate_epiphany(
    ...
    if context.participant_count >= 3 && context.collective_attunement > 0.0 {
        if let Some(mut outcome) = check_council_harmony(context.collective_attunement, context.participant_count, context.duration_ticks) {
            ...
            // === NEW: Proactive Joy on strong Council Harmony epiphanies (non-scar celebration) ===
            // In full ECS pass real player_id and registry
            // if let Some(registry) = /* access LegacyJournalRegistry */ {
            //     registry.generate_proactive_joy_redemption_thread(
            //         player_id,
            //         "Council Harmony bloom — joy from collective mercy".to_string(),
            //         outcome.abundance_bloom_multiplier * 4.0,
            //         0.3,
            //         /* current_tick */,
            //         /* server_id */,
            //     );
...
// ... (all other check_* and apply_* functions preserved)
...
// End of simulation/src/epiphany_catalyst.rs v18.97.1 + Proactive Joy
// Strong epiphanies (especially council harmony) now seed proactive joy threads.
// Thunder locked in. Yoi ⚡
