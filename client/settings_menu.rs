...
//! Beautiful, accessible, live-wired UI matching inventory_ui + divine_whispers_ui exactly
//! PATSAGi / Ra-Thor aligned labels, tooltips, and cosmic aesthetic
...
fn spawn_settings_menu(mut commands: Commands, asset_server: Res<AssetServer>) {
    commands
        .spawn((
            NodeBundle {
                style: Style {
                    ...
                    width: Val::Px(640.0),
                    height: Val::Px(720.0),
                    margin: UiRect::new(Val::Px(-320.0), Val::Auto, Val::Px(-360.0), Val::Auto),
        ...
        .with_children(|parent| {
            ...
            // FOV + Render Distance (simple +/- for v1)
            parent.spawn(TextBundle {
            ...
            // TODO: Full slider implementation can be added in next micro-iteration (drag logic similar to divine volume)
            // For now: display current + apply on button press
            ...
            // === SERVER RULES DISPLAY (beautiful panel) ===
            parent.spawn(TextBundle {
...
fn handle_settings_interactions(
    mut interaction_query: Query<(&Interaction, Option<&QualityPresetButton>, Option<&SettingsCloseButton>, Option<&SettingsApplyButton>, Option<&SettingsResetButton>), Changed<Interaction>>,
    mut menu_query: Query<&mut Visibility, With<SettingsMenuRoot>>,
    mut settings: ResMut<ClientSettings>,
    server_rules: Res<ServerRules>,
) {
    for (interaction, preset_btn, close_btn, apply_btn, reset_btn) in interaction_query.iter() {
        ...
        if let Some(preset) = preset_btn {
            ...
            // Live apply for graphics if possible
        }

        if apply_btn.is_some() {
            save_client_settings(&settings);
            // Trigger apply systems (they react to resource change)
            info!("[Settings] Applied & saved all changes (mercy aligned)");
        }

        if reset_btn.is_some() {
            *settings = load_client_settings(); // reloads mercy defaults
            info!("[Settings] Reset to PATSAGi-recommended defaults");
        }
    }
}

fn sync_menu_with_settings_resource(
    settings: Res<ClientSettings>,
    // Future: update text nodes, slider positions, toggle states here
) {
    if settings.is_changed() {
        // Sync UI state (text values, active preset highlight, etc.)
    }
}

// Helper to toggle menu visibility from pause menu or keybind (call this from external system)
