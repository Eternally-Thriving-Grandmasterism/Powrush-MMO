...
// RTS (Rauch-Tung-Striebel) Fixed-Lag Backward Smoother implementation (v18.37)
...
/// RTS (Rauch-Tung-Striebel) Fixed-Lag Smoother for 1D Kalman.
/// Stores recent forward-pass states and runs a backward recursion
/// to produce optimally smoothed estimates within the lag window.
#[derive(Clone, Debug)]
pub struct RTSFixedLagSmoother {
    pub smoothed_estimate: f32,
    history: Vec<RTSState>,   // recent forward states
    lag: usize,
}

#[derive(Clone, Debug)]
struct RTSState {
    estimate: f32,        // forward filtered estimate
    predicted: f32,       // one-step prediction
    covariance: f32,      // approximate error covariance P
    transition: f32,      // state transition (velocity influence)
}
...
impl RTSFixedLagSmoother {
    ...
    /// Update with new forward Kalman result.
    /// This performs the RTS backward pass on the current window.
    pub fn update(&mut self, new_estimate: f32, new_covariance: f32, dt: f32) {
        // Simple transition model: estimate_{k+1} ≈ estimate_k + velocity * dt
        // We approximate transition as 1.0 for position + small velocity carry
        let transition = 1.0;
        ...
        for i in (0..self.history.len() - 1).rev() {
            ...
            // RTS smoother gain
            let pred_cov = curr.covariance * curr.transition * curr.transition + 0.1; // process noise approx
...
// ============================================================
// SAFETY NET MONITORING SNAPSHOT (extended with RTS)
// ============================================================
...
pub struct SafetyNetMonitoringSnapshot {
    ...
    pub smoothed_latency: f32,           // from RTS or heuristic
...
// ============================================================
// CORE STRUCTS (abbreviated for space — full versions in previous commits)
// ============================================================
...
impl LatencyHistogram {
    ...
    pub fn p50(&self) -> u64 { 0 }
    pub fn p95(&self) -> u64 { 0 }
    pub fn p99(&self) -> u64 { 0 }
}
...
impl KalmanFilter1D {
    pub fn new(initial: f32) -> Self { /* ... */ Self { estimate: initial, velocity: 0.0, last_residual: 0.0, process_noise: 0.1, measurement_noise: 15.0, error_estimate: 1.0, error_velocity: 1.0 } }
    pub fn update(&mut self, m: f32, dt: f32) -> f32 { /* ... */ m }
}

#[derive(Clone, Debug)]
pub struct KalmanFilter2D { /* ... */ }
impl KalmanFilter2D { pub fn new(lat: f32, jit: f32) -> Self { /* ... */ } pub fn update(&mut self, m_lat: f32, m_jit: f32, dt: f32) {} }

#[derive(Clone, Debug)]
pub struct FixedLagKalmanSmoother { /* legacy heuristic */ pub smoothed_estimate: f32, history: Vec<f32>, lag: usize }
impl FixedLagKalmanSmoother { pub fn new(lag: usize) -> Self { Self { smoothed_estimate: 0.0, history: vec![], lag } } pub fn update(&mut self, v: f32) {} }

// ============================================================
// SAFETY NET STATE (with RTS Smoother)
// ============================================================
...
impl Default for SafetyNetState {
    fn default() -> Self {
        Self {
            ...
            kalman_latency: None, kalman_jitter: None, kalman_2d: None,
...
impl RbeClientSync {
    ...
    async fn handle_safety_net_broadcast(
        ...
        if safety.sample_count % 5 == 0 {
            ...
            tracing::info!(
                "[SafetyNet][RTS] RTS smoothed={:.1} | residual={:.1}",
                rts_val,
        ...
        if latency_ms > 150 || jitter_ms > 50 {
            tracing::warn!("[SafetyNet] High latency/jitter");
        }
