...
/// Load RON config (Startup). Replace default() with actual ron::from_str when loading from assets.
pub fn load_region_palette_config(mut commands: Commands) {
    let config = RegionPaletteConfig::new_with_defaults();
