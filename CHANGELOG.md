...
## [23.2.84] — 2026-09-09 — Stranger-loop seal notice

Near Heartwood Wards posts, the existing climate slab says `Wards · E tend · Well · Grove · Ember` and E tends dress. Soft post breath marks reach. No second HUD, no house-book write, no Places/Market/Sanctuary mesh. Title Online stays grey. `playable-preview` stays `11c577e`. Floor stays `2163551`. Workspace stays 21.88.0.

## [23.2.83] — 2026-09-09 — U8.1: pause-on-all-hexes

Esc from the yard now opens the same pause plate on Sanctuary, Heartwood, and any later local hex. One yard pause verb sits behind Esc, pad Start, key 3, and the touch pause chip; it reads the launch door and never the hex. An open Places plate walks back to pause instead of eating the key edge, and a confirmed `Leave this hex` lands on a bare yard so the plate from the hex before is not carried over. Places stays eligible on that plate after Settled + book. No second HUD and no new settings row. Title Online stays grey with no listen. `playable-preview` stays `11c577e`. Floor stays `2163551`. Workspace stays 21.88.0.

## [23.2.82] — 2026-09-08 — U8: Heartwood Wards

Heartwood now carries three seal posts (Well · Grove · Ember) outside the empty lamp disk and the water bath. Look and tend are session dress on the existing Peace Use. They do not write the house book, so `hour_three_complete`, the embassy seat, Places eligibility, and Sanctuary return stay held. Not a Places row. No Market, socket, public bind, or Title Online path. `playable-preview` stays `11c577e`. Floor stays `2163551`. Workspace stays 21.88.0.
