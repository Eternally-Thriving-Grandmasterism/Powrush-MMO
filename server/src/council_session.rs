...
impl CouncilSession {
    ...
    /// Server tick — updates collective bloom field
...
impl CouncilSessionManager {
    ...
    /// Main server tick — updates all sessions and returns sync events
    pub fn tick_all(&mut self, current_tick: u64) -> Vec<CouncilBloomSyncEvent> {
        ...
        // Close sessions that need to be closed
        for id in to_close {
            if let Some(session) = self.sessions.remove(&id) {
                // Fire-and-forget persistence recording (non-blocking)
