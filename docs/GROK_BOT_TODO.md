...
1. **MESH-PERSONA** — player + entities leave capsule greybox toward PersonaCommit / Place dress. **Only with Core asset budget.** No Title race select. Practices stay post-House dress (`PRACTICE_NOT_VERB`).
2. **B5** — Practice ≠ Verb candidate until Core names CARD (cite [`docs/PRACTICE_NOT_VERB.md`](PRACTICE_NOT_VERB.md)). Do not invent Hands paths.
3. **Wave C** — Wards later from current main.
4. **Art/mesh budget** / Hour-two/Steam — Core/human.

Hour-two minutes stay blank for the human. Title Online stays grey. Floor `2163551`. Tag `11c577e`. Workspace `21.88.0`.
