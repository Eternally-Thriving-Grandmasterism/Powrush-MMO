...
Refuse is **not** “never build an MMO.” It is “do not smash Product 2 chrome into Product 1 this tick.”
...
Core CARD → Hands one rung → clerk verifies PATHS → Core QA → HOLD → next CARD.
Never two rungs in flight. `OFFER NEXT` banned. Queue = GROK_BOT_TODO **Next** only.

QA: stranger boots, walks, **E** answers *this tick*. If the rung does not make that truer **or** open the next named phase without lying, do not merge.

Drive = refs (faces, buildings, stills). ART_BIBLE + five dresses. No sixth people. No binary cargo until a CARD names an existing path or one new file this CARD lists.

Wave C, B1–B7, Places, Comfort, Temper, Persona flags are closed. Do not rebuild.
...
One CARD. No OFFER NEXT. Wait Hands ACK + clerk PATH-check.
Use AGENT_QA_ORACLE.md schema.

## Standing orders — Bot 1 HANDS

Execute the Core CARD PATHS only.
If PATHS missing: HOLD, do not invent.
QA after merge: Play · Online grey · walk · E · Places×4 · Comfort L still readable.
Then HOLD.

## Standing orders — Bot 3 CLERK

Verify every CARD path on current tip.
If missing: `HOLD · no invent.`
Do not skip P2 to P7. Do not light Online.
Cite DEMO KEEP vs NEVER vs HOLD-until-P#.

One sentence: Refuse is a calendar, except for lies. Gold never teaches. Online waits for `online yes`. The well stays first.
