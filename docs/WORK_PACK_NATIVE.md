...
| Surface | Owns |
|---|---|
| **Powrush-MMO** | Hands, climate, satchel, allocate, House, Ledger, feel, local JSON |
...
## Wave 1 — already on main (verify, do not redo)

- First-take glow + camera punch + rumble (`harvest_feel` — do not rewrite)
- Well speech Idle / Glowing / Tended / Resting / Stressed
- Teaching claim *extract left it tired* / *flow restored the well* (`climate_script` + `lived_hour_bind`)
...
Surgical hooks only. No rewrite of `harvest_feel.rs` or `rbe_allocate_choice.rs`. Core gate: `shared` + `rsil-identity` + `powrush-client --lib`. Merge only if Core green.
