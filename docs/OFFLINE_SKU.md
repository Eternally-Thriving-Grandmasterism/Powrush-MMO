...
**Agent landing:** walked slices (U0–U8 including #308), Core gate, and do-not list live in README (dated section after Floor). This file is SKU law; do not reopen it or freelance a new product.
...
- **Continue** loads last hex **later** (U2, not this stamp). U0 does not implement Continue-to-last-hex.

## Places after Settled + book (disk only, later slices)

After Settled + book, later slices may add **disk-only** places. Do **not** ship five Sanctuary clones. Do **not** spawn Heartwood / Depths on Sanctuary.

| Place | Law |
|---|---|
| **Heartwood** | Peace. Later slice. Own hex file. |
| **Threshold** | look / tend. Later slice. Own hex file. |
| **Market** | spike **only if** lethal is declared **on that hex**. |
| **Depths** | one landing. Later slice. Own hex file. |

Sanctuary stays the yard. Later places are later hexes, not overlays on Prime.
...
## User-dir saves (install law — U1 implements)
...
- U0 documents the law. **U1 implements** the path. Do not pretend U1 shipped.
...
U1 moves these into the writable user dir. Install-dir-only is a fail.
...
| Slice | Job | Not U0 |
|---|---|---|
| **U1** | writable user-dir persist | implement, do not document-as-shipped |
| **U2** | Continue loads last hex | Play still boots Sanctuary |
| **U3.5** | leak tick | isolation stays until that named law |
...
- Five Sanctuary clones. Heartwood / Depths spawned on Sanctuary.
