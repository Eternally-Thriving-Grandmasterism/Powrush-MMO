...
Lived-tick ingest (`POWRUSH_INGEST`) is **off** on a first-hour Peace boot — L3 is absent until a human turns it on. `POWRUSH_NET` defaults **off** (zero sockets). G0 light gen is **off** by default — hex-seed scatter + climate fog behind UI; enable via Settings **Grove · light** or env `POWRUSH_GEN=light` (OR; same path) for F3 re-walk (safer lavapipe door).
