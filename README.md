...
**Floor:** stranger/play floor `2163551`; `playable-preview` tag @ walked SHA `11c577e` (steward preview yes; not the glow commit; later tip `3fc46c9` is welcome-back breath only — comfort, not a second launch); L1 hex sign on `7ea48328` is a sign after Settled + book, default off, not a second launch; Online grey; comfort + canons live; not a launch candidate.

**Agent landing (2026-09-09):** Steam Offline first. Title Online stays grey (no listen). LAN default off. `playable-preview` stays `11c577e` until a named Steam retag. Floor `2163551`. Hour-two minutes are human-only — do not invent them. Ra-Thor does not drive WASD. Core gate: `cargo test -p shared -p rsil-identity` and `cargo test -p powrush-client --lib`. Merge only if that Core is green and do-not-ship holds. PATSAGi + Ra-Thor council decides cuts. Agents collaborate; they do not freelance a new product. SKU law: `docs/OFFLINE_SKU.md`. Read `AGENTS.md` and `docs/PATSAGI_MERGE_COURT.md` before any slice. Fill the PR ballot. Label `patsgi-auto` only for AUTO. Steward steers STEER/HOLD. Title Online stays grey.

Already walked — do not rebuild: U0 docs #298 `65458c4`; U1 user-dir saves #299 `a5755e2`; U2 local hex travel #300 `5c754ad` (no-book Places closed, yard Sanctuary); U2 book-survives return #301 `0121433`; U3 Heartwood lamp spatial #302 `76adebe` (no buildings in water or lamp disk); U4 audio #303 `0b73734` (mute silences bed + well sting; no ALSA hang); U5 Deck Title #304 `789529e` (1280x800 click-clean); U6 Heartwood lip #305 `6759b80` (ribs outside disk, water is a bath, book stays); U7 Threshold shelf #307 `c269a89` (look + tend only, session-local); #306 Cloud Agent env (bootstrap only: `.cursor` install/xvfb/lavapipe, no secrets, no listen); #309 Lived-hour SP HourTwoHeld + NetMode `25aaf575` (Offline default; `title_online_enabled` always false). Human playtest of the Hour-two held card is still owed.

Do not merge or continue #308 (U8 Heartwood Wards). Core failed: `wards_are_valid` / `posts_are_heartwood_dress_outside_lamp_and_water`. Hold for Sherif. No Always allow. No public bind. No Title Online on. No Depths as boot. No Market. No Threshold as a second product. No second HUD / F-row. No Heartwood mesh on Sanctuary. Isolation gamma 0. Sanctuary isolated. Empty lamp disk is law. Three surfaces stay separate: this human game, Ra-Thor lattice, Powrush-MMO-Simulator browser. Do not fold the player loop into Ra-Thor.
...
Lived-tick ingest (`POWRUSH_INGEST`) is **off** on a first-hour Peace boot — L3 is absent until a human turns it on. `POWRUSH_NET` defaults **off** (zero sockets). Settings **LAN · off|loopback** is a loopback lab beside Grove (default **off**; unknown → off): loopback is **127.0.0.1** only, never a public bind, and never lights Title Online. Title Online stays grey. G0 light gen is **off** by default — hex-seed scatter + climate fog behind UI; enable via Settings **Grove · light** or env `POWRUSH_GEN=light` (OR; same path) for F3 re-walk (safer lavapipe door).
...
- `POWRUSH_GEN=light` or Settings **Grove · light** — optional G0 hex-seed scatter + climate fog (default **off**; OR; world Camera3d fog only; cull when Title/pause/Settings/L/Q open)