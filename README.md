...
**Floor:** stranger floor `2163551` / main tip `2163551`; comfort + canons live; not a launch candidate.
...
Lived-tick ingest (`POWRUSH_INGEST`) is **off** on a first-hour Peace boot — L3 is absent until a human turns it on. `POWRUSH_NET` defaults **off** (zero sockets). G0 light gen (`POWRUSH_GEN=light`) is **off** by default — hex-seed scatter + climate fog behind UI; set the env to enable for F3 re-walk (safer lavapipe door).
...
- `POWRUSH_GEN=light` — optional G0 hex-seed scatter + climate fog (default **off**; world Camera3d fog only; cull when Title/pause/Settings/L/Q open)